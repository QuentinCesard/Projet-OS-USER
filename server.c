     ...
     {    
        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
                ...
                if (nbClients==4)
				{
					...
					sprintf(reply, "D %d %d %d", deck[0], deck[1], deck[2]); //distribution des 3 premieres cartes
		...
		switch (buffer[0])
		{
                	...
                	case 'O':{
						...
						for (i=0;i<4;i++)
						{
							if (i!=id) //chercher uniquement chez les autres joueurs
							{
								if (tableCartes[i][objet]>0) //joueurs possède l'objet
								{
									sprintf(reply,"V %d %d %d",i,objet,100); //100=*
									broadcastMessage(reply);
								}
								else
								{
									sprintf(reply,"V %d %d %d",i,objet,0); //0=pas l'objet
									broadcastMessage(reply);
								}
							}
						}
					...
					case 'S':{
						...
						sprintf(reply,"V %d %d %d",cible,objet,tableCartes[cible][objet]); //affichage de la valeur
						broadcastMessage(reply);

						joueurCourant=(joueurCourant+1)%4; //joueur suivant
						sprintf(reply,"M %d",joueurCourant);
						broadcastMessage(reply);
					// RAJOUTER DU CODE ICI
						break;
